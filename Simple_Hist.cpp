...
H1i::H1i(char * Name, char * Title,int N_bins, double Min, double Max, char * X_axis_label, char * Dir){
	...
	float * temp_array = new float[N_bins];
...
H2i::H2i(const char * Name, char * Title,int X_N_bins, double X_Min, double X_Max, char * X_axis_label,int Y_N_bins, double Y_Min, double Y_Max, char * Y_axis_label, char * Dir){
...
void H2i::print_info(){
	...
	cout<< Yaxis->n_bins << ", "<< Yaxis->min<< ", "<< Yaxis->max<< ", "<< Yaxis->title<< ", "<< dir << ", hash key ="<< key <<endl;
}
////////////////////////////////////////////  H2i end  ////////////////////////////////////////////
...
void histo_handler::fill1(char * NAME, double x, char * TITLE, int N_BINS, double MIN, double MAX, char * X_LABEL, char * DIR){
	...
	if(h1i_map.count(K) == 0){
		...
		if (  false)//hist_check_counter < 10000 && !(h1i_map[NAME+DIR]->match(NAME, TITLE, N_BINS, MIN, MAX, X_LABEL, DIR)) )
		{
			Red(true);
			cout<<"ERROR: Histogram is:"<<NAME<< ", "<< TITLE<< ", "<< N_BINS<< ", "<< MIN<< ", "<< MAX<< ", "<< X_LABEL<< ", "<< DIR<<endl;
			cout<<"And it should be   :";
			h1i_map[K]->print_info();
			White(false);
		}
		else {
			h1i_map[K]->fill(x);
...
void histo_handler::combine_hist(H1i * hist2){
	...
	if(h1i_map.count(hist2->key) == 0){ //if it dosen't exit just add it
		h1i_map[hist2->key]=hist2;
	}
	else{
		if ( true ) //h1i_map[key]->match(hist2->get_name(), hist2->get_title(), hist2->get_X_n_bins(), hist2->get_X_min(), hist2->get_X_max(), hist2->get_X_title(), hist2->get_dir() ))
		{
		//if( *h1i_map[key] == *hist2){
			for(int i=0; i < hist2->Xaxis->n_bins; ++i){
				h1i_map[hist2->key]->bins[i] = h1i_map[hist2->key]->bins[i] + hist2->bins[i];
			}
			h1i_map[hist2->key]->set_X_overflow(  h1i_map[hist2->key]->get_X_overflow()  +	hist2->get_X_overflow() );
			h1i_map[hist2->key]->set_X_underflow( h1i_map[hist2->key]->get_X_underflow() +	hist2->get_X_underflow() );
		}
		else {
			Red(true);
			cout<<"ERROR in combine_hist()"<<endl;
			cout<<"ERROR: Histogram is:";	hist2->print_info();
			cout<<"   And it should be:";	h1i_map[hist2->key]->print_info();
...
void histo_handler::fill2(const char * NAME, double x, double y, char * TITLE, int X_N_BINS, double X_MIN, double X_MAX, char * X_LABEL, int Y_N_BINS, double Y_MIN, double Y_MAX, char * Y_LABEL, char * DIR){
	char key[500] = "";
	strcat(key,NAME);
	strcat(key,DIR);
	const char * K=key;
	//if (hist_check_counter >80 && hist_check_counter <90){
	//	string  testKEY=h2i_map[key]->key;
	//	cout << "hist_check_counter=" << hist_check_counter << endl;
	//	cout << "hash key =" << testKEY << ", current hash key =" << key << endl;
	//}

	if(h2i_map.count(K) == 0){
		H2i * hist = new H2i(NAME, TITLE, X_N_BINS, X_MIN, X_MAX, X_LABEL, Y_N_BINS, Y_MIN, Y_MAX, Y_LABEL, DIR);
		h2i_map[K]=hist;
		//printf("add new\n");
	}else{
		//checking to see if the histogram matches is slow so we will only do this in the very begining 
		if ( strcmp(TITLE, h2i_map[K]->get_title() ) != 0 )//hist_check_counter < 10000 &&  !(h2i_map[NAME+DIR]->match(NAME, TITLE, X_N_BINS, X_MIN, X_MAX, X_LABEL,  Y_N_BINS, Y_MIN, Y_MAX, Y_LABEL, DIR)) )
		{
			Red(true);
			cout<<"ERROR: Histogram is:"<<NAME<< ", "<< TITLE<< ", "<< X_N_BINS<< ", "<< X_MIN<< ", "<< X_MAX<< ", "<< X_LABEL<< ", "<< Y_N_BINS<< ", "<< Y_MIN<< ", "<< Y_MAX<< ", "<< Y_LABEL<< ", "<< DIR << ", hash key =" << key <<endl;
			cout<<"And it should be   :";
			h2i_map[K]->print_info();
			cout << "hist_check_counter=" << hist_check_counter << endl << endl;
			White(false);
		}
		else {
			h2i_map[K]->fill(x,y);
...
void histo_handler::combine_hist(H2i * hist2){
	...
	if(h2i_map.count(hist2->key) == 0){ //if it dosen't exit just add it
		h2i_map[hist2->key]=hist2;
	}
	else{
		if ( true )//h2i_map[key]->match(hist2->get_name(), hist2->get_title(), hist2->get_X_n_bins(), hist2->get_X_min(), hist2->get_X_max(), hist2->get_X_title(), hist2->get_Y_n_bins(), hist2->get_Y_min(), hist2->get_Y_max(), hist2->get_Y_title(), hist2->get_dir() ))
		{
			for(int i=0; i < (int)hist2->get_X_n_bins(); ++i){
				for(int j=0; j < (int)hist2->get_Y_n_bins(); ++j){
					h2i_map[hist2->key]->bins[i][j] = h2i_map[hist2->key]->bins[i][j] + hist2->bins[i][j];
				}
			}

			h2i_map[hist2->key]->set_X_overflow(  h2i_map[hist2->key]->get_X_overflow()  +	hist2->get_X_overflow() );
			h2i_map[hist2->key]->set_X_underflow( h2i_map[hist2->key]->get_X_underflow() +	hist2->get_X_underflow() );
			
			h2i_map[hist2->key]->set_Y_overflow(  h2i_map[hist2->key]->get_Y_overflow()  +	hist2->get_Y_overflow() );
			h2i_map[hist2->key]->set_Y_underflow( h2i_map[hist2->key]->get_Y_underflow() +	hist2->get_Y_underflow() );
			...
			cout<<"ERROR in combine_hist()"<<endl;
			cout<<"ERROR: Histogram is:";	hist2->print_info();
			cout<<"   And it should be:";	h2i_map[hist2->key]->print_info();
